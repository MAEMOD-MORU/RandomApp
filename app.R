...
# credentials = drop_read_csv(file.path(drop.folder, 'Credentials.csv'),fileEncoding = 'UTF-8-BOM')
# print(credentials)
...
# data headers: site, screeningID, age, sex, randomizationID, Treatment, Date, user
...
server <- function(input, output, session){
    ...
    observeEvent(input$validate.button,{
      ...
      if(values$check.screenID == T && values$check.age == T && between(values$age,18,50) ==T){  
        ask_confirmation(
          ...
          text = tags$div(
            #tags$b("Registered by: "),res_auth$user,
    ...
    data.table <- reactivePoll(1000, session,
                               ...
                               checkFunc = function() {
                                 data_name <- paste0("data-",res_auth$site,".csv")
                                 if (file.exists(file.path(drop.folder, data_name)))
                                   file.info(file.path(drop.folder, data_name))$mtime[1]
                                 else
                                   ""
                               },
                               ...
                               valueFunc = function() {
                                 data_name <- paste0("data-",res_auth$site,".csv")
                                 drop_read_csv(file.path(drop.folder, data_name))
                               }
    )
